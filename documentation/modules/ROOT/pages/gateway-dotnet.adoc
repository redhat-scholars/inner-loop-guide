...
This time we are still going to use OpenShift S2I, but we will invoke it using the OpenShift CLI (oc commands). 
Also, we will use S2I to compile the application and create the .NET artefact .dll before creating a container.
...
----
You can try this for yourself in the Gateway service pod by `*selecting the Gateway
