...
[NOTE]
====
Recently browsers such as *Google Chrome* have started converting all *HTTP* requests into *HTTPS* (secure) requests by 
...
Please check carefully in the URL bar for a padlock and edit the URL to remove any *'s'* to make the request HTTP (ie. Not Secure) 
and then retry. Its not the purpose of this simple workshop to demonstrate how OpenShift can leverage TLS security.
====
